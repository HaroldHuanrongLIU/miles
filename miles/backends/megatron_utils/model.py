...
def _grad_audit_family(name: str) -> str:
...
def _grad_audit_should_run() -> bool:
    value = os.environ.get("MILES_GRAD_AUDIT_ENABLE", "")
    return value.lower() in {"1", "true", "yes", "on"}


def _grad_audit_max_steps() -> int:
...
def _grad_audit_output_dir(args: Namespace) -> Path:
    return Path(os.environ.get("MILES_GRAD_AUDIT_DIR") or Path(args.save).parent / "grad_audit")


def _write_grad_audit(args: Namespace, rollout_id: int, step_id: int, model: Sequence[DDP]) -> None:
    if not _grad_audit_should_run():
        return

    max_steps = _grad_audit_max_steps()
    ...
    for chunk_idx, model_chunk in enumerate(model):
        for name, param in model_chunk.named_parameters():
            ...
            family_name = _grad_audit_family(name)
            ...
            family["max_abs_grad"] = max(float(family["max_abs_grad"]), grad_max)
            if grad_max == 0.0:
                zero_grad += 1
                family["zero_grad"] += 1
    ...
    output_dir = _grad_audit_output_dir(args)
...
def train_one_step(
    ...
    dumper_phase_util.finalize(model)
    _write_grad_audit(args, rollout_id, step_id, model)
